  ...
  return (
    <form onSubmit={handleSubmit} className="flex w-full max-w-sm flex-col gap-2 sm:flex-row">
      <input
        ...
        required
        autoFocus
        className="flex-1 rounded-xl border border-primaryText/10 bg-white px-4 py-2.5 text-sm text-primaryText placeholder:text-secondaryText/50 focus:border-accent/50 focus:outline-none focus:ring-2 focus:ring-accent/30"
      />
      <button
        type="submit"
        disabled={status === "loading"}
        className="rounded-xl bg-primaryText px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#0b223d] disabled:opacity-60"
      >
